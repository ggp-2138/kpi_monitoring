...
[![CI/CD Status](https://github.com/ggp-2138/kpi_monitoring/actions/workflows/g1.yml/badge.svg)](https://github.com/ggp-2138/kpi_monitoring/actions/workflows/g1.yml)
...
基于自建私有 **Self‑Hosted Runner** 搭建完整工作流，代码提交 main 分支自动触发全流程：

1. 代码拉取、Python环境初始化

2. black代码格式化校验

3. MAD异常检测算法单元测试（pytest）

4. Docker镜像构建、推送至Docker‑Hub

5. SSH远程登录服务器，强制同步远端最新配置

6. 零停机更新业务容器

- 流水线耗时：2分钟内完成整套自动化部署

- 实现效果：代码提交即自动上线，省去人工打包、部署操作

**CI/CD 配置文件路径：**[.github/workflows/g1.yml](.github/workflows/g1.yml)

**最终成果**：落地“代码提交 → 自动构建 → 穿透内网 → 容器自愈”全链路，部署总时长稳定在 2分钟

### 10.1 CI/CD流程图
```
[开发者] git push main 分支
       │
       ▼
┌──────────── [ GitHub Actions 触发 ] ────────────┐
│ 1. 拉取代码 / 配置 Python 3.11                  │
│ 2. 安装依赖 & Black 格式校验                   │
│ 3. MAD 异常检测算法 单元测试 (pytest)          │
└────────────────────┬───────────────────────────┘
                     │ (测试通过)
                     ▼
        ┌──────────────────────┐
     ...
     ┌────────────┴────────────┐
     ▼                         ▼
[首次部署] git clone ...   [更新] cd test_cicd && git pull
     │                         │
     └────────────┬────────────┘
                  ▼
       ┌─────────────────────────┐
       ...
       └──────────┬──────────────┘
                  ▼
       ✅ 容器拉取完成, 服务全 Up (耗时 2 分种)

```
### 10.2 内网穿透远程部署架构（基于 frp）

由于业务服务器处于 **NAT 网络环境**（如家用网络、手机热点），不具备可直接从公网访问的固定 IP，导致云端的 GitHub Actions 无法通过常规 SSH 直连部署

为此，我采用 **frp (Fast Reverse Proxy)** 构建了内网穿透隧道，解决了跨网络隔离的自动化部署难题

**网络架构流向图：**

```
┌─────────────────┐      SSH 请求      ┌────────────────────────┐
│ GitHub Actions  │ ────────────────> │  阿里云 ECS 实例         │
│ (Self-hosted    │                   │  (frps 服务端, 端口:7000)│
│  Runner)        │                   └────────────────────────┬┘
└─────────────────┘                                            │
                       隧道映射 (TCP/UDP 穿透)                 │
                              │                                │
...
```
- **服务端 (frps)**：部署在有公网 IP 的阿里云香港服务器上，监听 7000 端口，作为内网穿透的入口。配置了 token 鉴权认证，防止恶意扫描和非法连接

- **客户端 (frpc)**：部署在本地 Ubuntu 业务虚拟机中。通过配置，将本地内网的 22 端口（SSH） 映射到阿里- 云服务器的 7001 端口

- **CI/CD 自动化交付**：在 GitHub Actions 部署脚本中，将 SSH 连接的目标 IP 设为阿里云香港服务器 IP，连接端口设为 7001。当流水线触发时，外网发往 7001 端口的请求会自动通过 frp 隧道，精准穿越 NAT，直达本地虚拟机的 22 端口，并执行后续的 docker compose up 部署命令

### 10.2 📌 CI/CD 踩坑复盘

- **1. 权限与密钥**：Runner 普通用户无 Docker 权限，私钥漏贴头尾标识符
解决：添加用户到 docker 组并重启 Runner；强制完整复制 id_rsa 首尾标识符

- **2. 网络穿透**：手机热点 NAT 隔离，云端无法直连本地内网 22 端口（i/o timeout）
解决：基于 frp 建立 TCP 隧道，配置 token 鉴权防止非法扫描，开启 heartbeat 保活应对热点网络波动

- **3. 配置与残留**：容器启动后全部 Exited。排查发现挂载的 prometheus.yml 等配置文件未进 Git 版本库；且 Docker Compose 默认使用文件夹名作项目名，导致旧容器冲突
解决：将编排文件与依赖配置同源管理（全部 Git 提交）；部署命令显式指定 -p 锁定项目名，避免环境残留污染
